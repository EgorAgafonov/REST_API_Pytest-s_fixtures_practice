...
def tests_pdf_test_report_maker():
    ...
    pdf_report.drawString(25, 700, f"Тест: <ИМЯ ТЕСТА>")
    # Generate test result:
    pdf_report.setFillColor('Gainsboro')
    pdf_report.roundRect(x=20, y=650, width=555, height=40, radius=5, stroke=0, fill=1)
    pdf_report.setFillColor('YellowGreen')
    pdf_report.roundRect(25, 655, 30, 30, 5, fill=1, stroke=0)
    pdf_report.setFillColor('Black')
    pdf_report.setFont('Arial_Bold', 16)
    pdf_report.drawString(60, 665, "PASSED   [100%]")
    pdf_report.setFillColor('Gainsboro')
    pdf_report.roundRect(x=20, y=440, width=555, height=200, radius=5, stroke=0, fill=1)
    pdf_report.setFillColor('Black')
    pdf_report.setFont('Arial_Cyr', 12)
    pdf_report.drawString(25, 625, f"""Идея:'<ОПИСАНИЕ ТЕСТА>'""")
    pdf_report.save()